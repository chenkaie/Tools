...
$col_make_enter =   $col_enter;
$col_make_leave =   $col_leave;
...
#	}
	...
	elsif ($thisline =~ /^(\(|\[|a(r|wk)|c(p|d|h(mod|own))|do(ne)?|e(cho|lse)|f(ind|or)|i(f|nstall)|mv|perl|r(anlib|m(dir)?)|s(e(d|t)|trip)|tar|Compile)\s+/)
	{
		$in = $1;
	}
	elsif ($thisline =~ /^((p|g)?make\[.*Entering directory)/)
    {
        $thisline =~ s/^((p|g)?make\[)/$col_make_enter$1/x;
    }
	elsif ($thisline =~ /^((p|g)?make\[.*Leaving directory)/)
    {
        $thisline =~ s/^((p|g)?make\[)/$col_make_leave$1/x;
    }
